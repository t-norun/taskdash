...
export default function AdminPlatformBalance({ getPlatformBalance }) {
  ...
  const parseUsd = (res) => {
    ...
    if (res) {
      usd =
        pick(res.balanceUsd) ??
        pick(res.platformBalanceUsd) ??
        pick(res.availableUsd);
      ...
      if (usd == null && platformBalanceCents != null) {
        usd = platformBalanceCents / 100;
      }

      if (usd == null && pick(res.balance) != null) {
        usd = pick(res.balance);
      }
    }
  ...
  return (
    <div className="bg-white border border-[#E7E7E7] rounded-2xl p-6">
      ...
      <div className="mt-5 rounded-xl border border-[#F1F1F1] bg-white p-5">
        {err ? (
          <div className="rounded-xl border border-[#EF4444] bg-[#FEF2F2] px-4 py-3 text-[13px] text-[#991B1B]">
            {err}
          </div>
        ) : (
          <>
